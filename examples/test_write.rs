...
fn main() {
    ...
    let _: Result<(), MpiError> = sched.block_on(async {
        ...
        com.coll().barrier().await?;
        let num_procs = com.coll().num_procs();
        let this_proc = com.coll().this_proc_id();
        let buf: usize = (this_proc+1) * 100;
        let dest_proc = (this_proc + 1) % num_procs;
        unsafe {
            com.rma()
                .buf_write(SendPtr::new(&buf as *const usize), mem.prptr(dest_proc, 0), 1)
        }
        .await?;
        com.coll().barrier().await?;
